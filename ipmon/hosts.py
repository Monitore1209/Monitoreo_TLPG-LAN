...
def add_hosts():
    ...
    if request.method == 'GET':
        ...
        if form.validate_on_submit():
            ...
            threads = []
            for address in form.ip_address.data.split('\n'):
                ip_hostname = ""
                ip_address = address.strip()
                if address.find(','):
                    ip_address, ip_hostname = address.split(',')
                ip_hostname = ip_hostname.strip()
                ...
                threads.append(
                    pool.apply_async(_add_hosts_threaded, (ip_address, ip_hostname, ))
                )
...
# Private Functions ##
######################
def _add_hosts_threaded(ip_address, ip_hostname):
